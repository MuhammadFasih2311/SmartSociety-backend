...
export const getResidentBookings = async (req, res) => {
  try {
    console.log('📥 Fetching bookings for resident:', req.user?._id);
    ...
    const formattedBookings = bookings.map(booking => {
      let amenityId = booking.amenityId;
      if (!amenityId) {
        console.log('⚠️ Booking missing amenityId:', booking._id);
        const amenity = Amenity.findOne({ name: booking.amenityName, isActive: true });
        if (amenity) {
    ...
    console.log('📤 Sending bookings:', formattedBookings.length);
